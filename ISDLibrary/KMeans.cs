...
public class KMeans
{
    ...
    public OccurenceMatrix data;  // data to be clustered
    ...
    public KMeans(int K, OccurenceMatrix data, string initMethod, int maxIter, int seed)
    {
        this.K = K;
    ...
    private static void InitPlusPlus(OccurenceMatrix data,
        ...
        for (int k = 1; k < K; ++k) // find each remaining mean
        {
            ...
            for (int i = 0; i < N; ++i) // for each data item
            {
                ...
                for (int ki = 0; ki < k; ++ki)
                    distances[ki] = EucDistance(data.Matrix, means[ki], i);
            ...
            for (int j = 0; j < dim; ++j)
                means[k][j] = data.Matrix[newMeanIdx,j];
        } // k remaining means
    ...
    private static void UpdateMeans(double[][] means,
      OccurenceMatrix data, int[] clustering)
    {
        ...
        for (int i = 0; i < N; ++i)  // each data item
        {
            ...
            for (int j = 0; j < dim; ++j)
                result[cid][j] += data.Matrix[i,j];  // accumulate
        }
    ...
    private static bool UpdateClustering(int[] clustering,
      OccurenceMatrix data, double[][] means)
    {
        ...
        for (int i = 0; i < data.Length; ++i)  // each data item
        {
            ...
            for (int k = 0; k < K; ++k)
                dists[k] = EucDistance(data.Matrix, means[k], i);
    ...
    private static double ComputeWithinClusterSS(OccurenceMatrix data,
        double[][] means, int[] clustering)
    {
        ...
        for (int i = 0; i < data.Length; ++i)
        {
            ...
            sum += SumSquared(data.Matrix, means[cid], i);
        }
        return sum;
    }

    private static double SumSquared(double[,] item, double[] mean, int i)
    {
